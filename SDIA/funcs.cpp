...
void buildptable(table *t)
{
	...
	for (int i = 0; i < t->n; i++) // Âûâîä òàáëèöû íà ýêðàí
...
{
   for (int i = 0; i < p.n; i++) // Ñîçäà¸ì äâîè÷íîå äåðåâî ïîèñêà
	  treeadd(t, *p.cont[i]);
}
...
{
   for (int i = 0; i < pt.n; i++) // Ñîåäèíÿåì òàáëèöû
...
void combine(elem x, table *t, bool a)
{
   if (a) // true èëè false
   {
	  for (int i = 0; i < t->n; i++) // Â ïåðâóþ òàáëèöó çàïèñûâàåì ñòîëáåö ñ öåíàìè
